...
js中除了使用`function`关键字创建函数外，还有一种很少见的方法，`new Function`，但是很少使用，不过也需要了解一下。
...
我们在学习闭包时，知道了词法环境的概念，js中，每次创建函数时，引擎默认都会使用`[[Environment]]`记录当前函数创建的外部环境。但是如果我们使用 `new Function` 创建一个函数，那么该函数的 `[[Environment]]` 并不指向当前的词法环境，而是指向全局环境。所以，记得当时我们说，js中的所有函数都是天然闭包的，不过排除了`new Function`的情况。
...
[https://zh.javascript.info/new-function](https://zh.javascript.info/new-function)
